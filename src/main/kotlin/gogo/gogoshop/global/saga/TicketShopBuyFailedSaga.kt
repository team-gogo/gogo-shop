...
class TicketShopBuyFailedSaga(
    ...
    @Transactional
    fun cancelledTicketBuy(miniGameId: Long, ticketType: TicketType, shopReceiptId: Long) {
        if (ticketType == TicketType.PLINKO) {

            val plinko = plinkoRepository.findByIdOrNull(miniGameId)
                ?: throw ShopException("Plinko Not Found -- SAGA.cancelledTicketBuy(miniGameId=$miniGameId, ticketType=$ticketType, shopReceiptId=$shopReceiptId)",
                    HttpStatus.NOT_FOUND.value()
                )

            val shopReceipt = shopReceiptRepository.findByIdOrNull(shopReceiptId)
                ?: throw ShopException("ShopReceipt Not Found -- SAGA.cancekkedTicketBuy(miniGameId=$miniGameId, ticketType=$ticketType, shopReceiptId=$shopReceiptId)",
                    HttpStatus.NOT_FOUND.value()
                )
            ...
            val yavarwee = yavarweeRepository.findByIdOrNull(miniGameId)
                ?: throw ShopException("Yavarwee Not Found -- SAGA.cancelledTicketBuy(miniGameId=$miniGameId, ticketType=$ticketType, shopReceiptId=$shopReceiptId)",
                    HttpStatus.NOT_FOUND.value()
                )
            ...
            val coinToss = coinTossRepository.findByIdOrNull(miniGameId)
                ?: throw ShopException("CoinToss Not Found -- SAGA.cancelledTicketBuy(miniGameId=$miniGameId, ticketType=$ticketType, shopReceiptId=$shopReceiptId)",
                    HttpStatus.NOT_FOUND.value()
                )
